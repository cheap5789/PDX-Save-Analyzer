...
>
> ⚠️ `country_name` has a small override vocabulary:
>
> - **Plain string override pointing at a TAG loc key** (e.g. `CHI`'s value is `"CHE"` → `loc["CHE"]` → "Chén").
> - **Plain string override pointing at a location slug** — used by the 105 placeholder-tagged minor nations (95 `AAA*` + 10 `ABA*` in the Bavaria autosave). Example: `AAA22` has `country_name == "sumbawa_province"` → `loc["sumbawa_province"]` → "Sumbawa". These are auto-generated independent minors, **not** colonial subjects. See `docs/games/eu5/duplicate-tags.md` for the full investigation.
> - **Dict form** `{name, key: {Adjective}, bases: {Base}}` for scripted display names like the horde civil-war pretender template. When the inner `name` points at a scripted loc entry (e.g. `NORTHERN_YUA: "Northern $YUA$"`) the value must be resolved via `resolve_scripted_value` against the regular loc dict, with `$ADJ$` substituted from the parent TAG's `_ADJ` loc key.
>
> All of this is handled centrally by `EU5Save.resolve_country_display_name(country_id)` — always prefer that method over a raw `loc.get(tag)`. The resolver assumes `save.loc` includes `location_names/location_names_l_english.yml`; `load_localisation` pulls that file in explicitly (see its docstring).
